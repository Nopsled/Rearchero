...
var ntohs = new NativeFunction(Module.findExportByName(null, 'ntohs'), 'uint16', ['uint16']);
var inet_addr = new NativeFunction(Module.findExportByName(null, 'inet_addr'), 'int', ['pointer']);
var connect = new NativeFunction(Module.findExportByName(null, 'connect'), 'int', ['int', 'pointer', 'uint']);
...
function replaceGadp() {
    Interceptor.replace(getaddrinfoPtr, new NativeCallback(function (name, service, req, pai) {
        const nameStr = Memory.readUtf8String(name)
...
function replaceConnect() {
    Interceptor.replace(connectPtr, new NativeCallback(function (socket, address, addressLen) {
        ...
        var ip = []
        const ipPtr = ptr(parseInt(address) + 4)
        const portPtr = ptr(parseInt(address) + 2)
        const port = ntohs(portPtr.readU16())
        endpoint.port = port

        if (port === 443) {
            Memory.writeInt(ipPtr, inet_addr(Memory.allocUtf8String("10.0.1.9")));
        }
...
console.log('[Networking.js]: Script loaded!')